...
# Configuration
VIDEO_DIRECTORY = os.environ.get("VIDEO_DIRECTORY", r"D:\Materials\Videos\CleverGet\Video")
        ...
        for filename in os.listdir(VIDEO_DIRECTORY):
            ...
            if os.path.isfile(filepath):
                ...
                if ext in valid_extensions:
                    ...
                    subtitle_path = os.path.join(VIDEO_DIRECTORY, f"{basename}-subtitles.vtt")
                    thumbnail_path = os.path.join(VIDEO_DIRECTORY, f"{basename}-thumbnails.vtt")
                    
                    # Check for cover image (same basename with .jpg extension)
                    cover_path = f"{basename}.jpg"
                    has_cover = os.path.isfile(os.path.join(VIDEO_DIRECTORY, cover_path))
...
def index():
    ...
    return render_template('index.html', videos=videos, video_directory=VIDEO_DIRECTORY)
...
def upload_file(video_basename, file_type):
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            file.save(filepath)
            return jsonify({'success': True, 'message': f'{file_type.capitalize()} file uploaded successfully'})
        except Exception as e:
...
def get_subtitles(video_basename):
    ...
    subtitle_path = os.path.join(VIDEO_DIRECTORY, f"{video_basename}-subtitles.vtt")
...
def translate_text():
    ...
    try:
        ...
        if remember_lang:
            ...
            logger.debug(f"Saved target language preference: {target_language}")
...
def explain_word():
    ...
    try:
        ...
        explanation = {
            'word': word,
            'meaning': word_obj.meaning if hasattr(word_obj, 'meaning') else 'Keine Bedeutung gefunden',
        }
        ...
        if "Connection" in str(e):
            ...
            return jsonify({'success': False, 'error': 'Konnte keine Verbindung zu Duden herstellen'})
        else:
